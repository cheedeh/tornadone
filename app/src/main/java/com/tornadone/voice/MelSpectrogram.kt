...
object MelSpectrogram {
    ...
    // Slaney mel scale constants (used by librosa and Whisper)
    private const val MEL_FREQ_SPACING = 200.0 / 3.0
    private const val MEL_MIN_LOG_HZ = 1000.0
    private val MEL_MIN_LOG_MEL = MEL_MIN_LOG_HZ / MEL_FREQ_SPACING
    private val MEL_LOG_STEP = ln(6.4) / 27.0

    private fun hzToMel(hz: Double): Double = if (hz >= MEL_MIN_LOG_HZ) {
        MEL_MIN_LOG_MEL + ln(hz / MEL_MIN_LOG_HZ) / MEL_LOG_STEP
    } else {
        hz / MEL_FREQ_SPACING
    }

    private fun melToHz(mel: Double): Double = if (mel >= MEL_MIN_LOG_MEL) {
        MEL_MIN_LOG_HZ * exp(MEL_LOG_STEP * (mel - MEL_MIN_LOG_MEL))
    } else {
        MEL_FREQ_SPACING * mel
    }
}
