...
class OnnxGestureClassifier @Inject constructor(
    ...
    fun reload() {
        ...
        if (path.isNotEmpty() && File(path).exists()) {
            try {
                Log.i("OnnxGestureClassifier", "Loading gesture model: $path")
                session = env?.createSession(File(path).readBytes())
            } catch (e: Exception) {
                Log.e("OnnxGestureClassifier", "Failed to load gesture model", e)
            }
        } else {
            Log.w("OnnxGestureClassifier", "No gesture model available — download required")
        }
    }
    ...
    private fun classify(): Long {
        return try {
            ...
            val currentEnv = env ?: return 1L
            val currentSession = session ?: return 1L

            OnnxTensor.createTensor(currentEnv, arrayOf(features)).use { inputTensor ->
                currentSession.run(mapOf("features" to inputTensor)).use { results ->
                    val raw = results.get("output_label").get().value
                    when (raw) {
                        is LongArray -> raw[0]
                        is Array<*> -> LABEL_NAMES.indexOf(raw[0].toString()).toLong()
                            .let { if (it < 0) 1L else it } // fallback to "none"
                        else -> 1L
                    }
                }
            }
        } catch (e: Exception) {
            Log.e("OnnxGestureClassifier", "Classification failed", e)
            1L // return "none"
        }
    }
